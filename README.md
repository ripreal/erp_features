...
Давайте  разберем последователньость этапов, который будет выполнять наш мега-пайплайн:
...
Теперь самое время перейти в реальному запуску пайплайна. Для этого в меню с пайплайном erp_features нажимаем кнопку Build with parameters и видим список параметров, которые нам нужно заполнить. Значения всех введенных параметров будут сохранены для следующих запусков пайплайна:
...
Чтобы лицензреть в отчета allure вот такие вот полезные скриншоты в случае падения шагов теста, нам потребуется установить утилитку nircmdm. Возьмем на карандащ также то, что снятие скриншотов будет работать только под slave нодой, запущенной в консольном режиме.
...
* Проверьте, что через cmd.exe у нас появилсь новая команда nircmd. Если все ок, то больше никакой настройки в дженкинсе не требуется/