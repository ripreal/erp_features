...
Функция Инициализация()
    ...
    Лог = Логирование.ПолучитьЛог("updateExtension");
    ЛОГ.Отладка("СЕРВЕР = " + СЕРВЕР);
    ЛОГ.Отладка("ПЛАТФОРМА_ВЕРСИЯ = " + ПЛАТФОРМА_ВЕРСИЯ);

КонецФункции

Процедура ОбновитьРасширение()

    Параметры = Конфигуратор.ПолучитьПараметрыЗапуска();
    Параметры.Добавить("/UpdateDBCfg -Extension " + ИМЯ_РАСШИРЕНИЯ);
...
Инициализация();
Лог.Информация("Updating extension %1 in DESIIGNER mode %2", ИМЯ_РАСШИРЕНИЯ, БАЗА);
ОбновитьРасширение();
Лог.Информация("Sucessfuly updated extension %1", ИМЯ_РАСШИРЕНИЯ);