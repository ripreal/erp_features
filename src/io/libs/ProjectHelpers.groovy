...
def dropDb(server1c, agentPort, serverSql, base, admin1cUser, admin1cPwd, sqluser, sqlPwd, fulldrop = false) {
    ...
    //}
    returnCode = utils.cmd("${env.WORKSPACE}/copy_etalon/drop_db.bat ${server1c} ${agentPort} ${serverSql} ${base} ${admin1cUserLine} ${admin1cPwdLine} ${sqluserLine} ${sqlpasswLine} ${fulldropLine}")
    if (returnCode != 0) { 
        error "error when deleting base with COM ${server1c}\\${base}. See logs above fore more information."
    }
}
...
def loadCfgFrom1CStorage(storageTCP, storageUser, storagePwd, connString, admin1cUser, admin1cPassword, platform) {
    ...
    returnCode = utils.cmd("runner loadrepo --storage-name ${storageTCP} --storage-user ${storageUser} ${storagePwdLine} --ibconnection ${connString} --db-user ${admin1cUser} --db-pwd ${admin1cPassword} --v8version ${platform}")
    if (returnCode != 0) {
